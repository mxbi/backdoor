...
class IMDBWiki(dataset.Dataset):
    ...
    """
    def __init__(self, image_shape=(48, 48), n_classes=50):
    ...
    def _download_cache_data(self):
        ...
        images = []
        labels = []
        for filename, label in zip(tqdm(filenames), all_labels):
        ...
        np.savez(f'{self.base_path}/data_{self.image_shape}_{self.n_classes}.npz', x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test)
